...
class ApiHandler {
  ...
  _connectionCheck() async {
    ...
    if (locStor.getString('con_type') == '2') {
      _apiurl = 'https://cmyo.sibegawan.com/api/v1';
      // _apiurl = 'http://192.168.0.112:4444/api/v1';
    }
...
// }
  ...
  checkCon() async {
    ...
    await _fakecheck();
    await _getappver();
    String fullUrl = 'https://cmyo.sibegawan.com/api/v1';
    SharedPreferences locStor = await SharedPreferences.getInstance();
    if (locStor.getString('con_type') == '2') {
      fullUrl = 'https://cmyo.sibegawan.com/api/v1';
    }
    await _getToken();
    var res = await http
        .get(Uri.parse(fullUrl), headers: _setHeaders())
