  ...
  void _login() async {
    ...
    if (_constat == 0) {
      AwesomeDialog(
        ...
        title: 'INFO',
        desc:
            'Login hanya bisa dilakukan saat indikator koneksi berwarna hijau',
      ).show();
    } else if (!_isLoading) {
      setState(() {
        _isLoading = true;
...
// }
  void _chekAuth() async {
    ...
    SharedPreferences locStor = await SharedPreferences.getInstance();
    String? token = locStor.getString('token');
    String? userStr = locStor.getString('user');

    print("Token saat startup: $token");

    // Jika tidak ada token atau user, minta login manual
    ...
    // ✅ Ada token → langsung masuk tanpa perlu login ulang
    // Gunakan data user yang tersimpan di SharedPreferences
    try {
      ...
      // Kalau data user corrupt, minta login ulang
      print("Data user corrupt, minta login ulang: $e");
